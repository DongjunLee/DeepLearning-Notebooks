...
#		self.l2['activation'] = l2_layer['activation']
	...
	def forward(self, X):
		...
		return l1_act, l2_pre_act, output
	...
	def backward(self, X, y, learning_rate, reg):
		...
		L1_score, _, output = self.forward(X)
		output[(np.arange(num_train), y)] = output[np.arange(num_train), y] - 1

		self.l2['W'] -= learning_rate * ( (L1_score.T).dot(output) + reg * (self.l2['W']) )
		self.l2['b'] -= learning_rate * np.sum(output/num_train, axis=0)
					
		dJdL1_score = (output/num_train).dot(self.l2['W'].T)
		dJdL1_score[L1_score == 0] = 0
			
		self.l1['W'] -= learning_rate * ( (X.T).dot(dJdL1_score) + reg * (self.l1['W']) ) 
		self.l1['b'] -= learning_rate * np.sum(dJdL1_score, axis=0)
